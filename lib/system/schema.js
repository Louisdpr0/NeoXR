module.exports = (m, env) => {
   ...
   if (setting) {
      ...
      if (!('sk_pack' in setting)) setting.sk_pack = 'ZeniBot'
      if (!('sk_author' in setting)) setting.sk_author = 'KSR CLOUD'
      ...
      if (!('owners' in setting)) setting.owners = ['89653898291']
      if (!isNumber(setting.lastReset)) setting.lastReset = new Date * 1
      if (!('msg' in setting)) setting.msg = 'Hi +tag 🪸\nI Saya Zenibot, Bot whatsapp yang akan membantu mu. Gunakan perintah dibawah ini dan jangan spam!'
      if (!isNumber(setting.style)) setting.style = 4
      if (!('cover' in setting)) setting.cover = 'https://i.ibb.co/PM1NBLz/default.jpg'
   } else {
      global.db.setting = {
         autodownload: true,
         debug: false,
         ...
         msg: 'Hi +tag 🪸\nI Saya Zenibot, Bot whatsapp yang akan membantu mu. Gunakan perintah dibawah ini dan jangan spam!',
         style: 4,
         cover: 'https://i.ibb.co/PM1NBLz/default.jpg',
      }
   }
}